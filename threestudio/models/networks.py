...
class DECORGAN(nn.Module):
    def __init__(self, dim_in: int, dim_out: int, config: dict):
        super().__init__()
        self.dim_hidden = config["n_neurons"]

        self.conv_0 = nn.Conv3d(dim_in,              self.dim_hidden * 1, 3, stride=1, dilation=1, padding=1, bias=True)
        self.conv_1 = nn.Conv3d(self.dim_hidden * 1, self.dim_hidden * 2, 3, stride=1, dilation=2, padding=2, bias=True)
        self.conv_2 = nn.Conv3d(self.dim_hidden * 2, self.dim_hidden * 4, 3, stride=1, dilation=2, padding=2, bias=True)
        self.conv_3 = nn.Conv3d(self.dim_hidden * 4, self.dim_hidden * 4, 3, stride=1, dilation=1, padding=1, bias=True)
        self.conv_4 = nn.Conv3d(self.dim_hidden * 4, self.dim_hidden * 8, 3, stride=1, dilation=1, padding=1, bias=True)

        self.conv_5 = nn.ConvTranspose3d(self.dim_hidden * 8, self.dim_hidden * 4, 4, stride=2, padding=1, bias=True)
        self.conv_6 = nn.Conv3d(self.dim_hidden * 4,          self.dim_hidden * 4, 3, stride=1, padding=1, bias=True)
        self.conv_7 = nn.ConvTranspose3d(self.dim_hidden * 4, self.dim_hidden * 2, 4, stride=2, padding=1, bias=True)
        self.conv_8 = nn.Conv3d(self.dim_hidden * 2,          dim_out,             3, stride=1, padding=1, bias=True)
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu(self.conv_0(x), negative_slope=0.02, inplace=True)
        x = F.leaky_relu(self.conv_1(x), negative_slope=0.02, inplace=True)
        x = F.leaky_relu(self.conv_2(x), negative_slope=0.02, inplace=True)
        x = F.leaky_relu(self.conv_3(x), negative_slope=0.02, inplace=True)
        x = F.leaky_relu(self.conv_4(x), negative_slope=0.02, inplace=True)

        x = F.leaky_relu(self.conv_5(x), negative_slope=0.02, inplace=True)
        x = F.leaky_relu(self.conv_6(x), negative_slope=0.02, inplace=True)
        x = F.leaky_relu(self.conv_7(x), negative_slope=0.02, inplace=True)
        x = self.conv_8(x)
        x = self.output_activation(x)
        return x


def get_conv(n_input_dims, n_output_dims, config) -> nn.Module:
    network: nn.Module
    if config.otype == "DECORGAN":
        network = DECORGAN(n_input_dims, n_output_dims, config_to_primitive(config))
    else:
