...
from threestudio.data.binvox_rw import (
    ...
    get_vox_from_binvox,
    write_ply_triangle,
    write_ply_triangle_color,
    get_simple_coarse_voxel,
)


@threestudio.register("voxel-grid")
class VoxelGrid(BaseImplicitGeometry):
    ...
    def configure(self) -> None:
        ...
        data_dir = os.path.join("./threestudio/data/decorgan", self.cfg.category, "train")
        
        self.training = False
        if self.training:
            # ----------------------------- this block for reading voxels from dir -----------------------------
            threestudio.info(f"Loading coarse voxels from {data_dir} ...")
            self.grids = []
            self.masks = []
            for shape in os.listdir(data_dir):
                try:
                    downsample_rate = self.cfg.downsample_rate
                    voxel = get_vox_from_binvox_512(os.path.join(data_dir, shape, "model_depth_fusion.binvox"))  # (512, 512, 512)
                except:
                    downsample_rate = self.cfg.downsample_rate // 2
                    data_dict = h5py.File(os.path.join(data_dir, shape, "model_depth_fusion.hdf5"), 'r')  # (256, 256, 256)
                ...
                mask_tensor = F.interpolate(coarse_voxel_tensor, scale_factor=self.cfg.upsample_rate, mode='nearest')
                coarse_voxel = coarse_voxel_tensor.detach().cpu().numpy()[0, 0]
                coarse_voxel = np.round(coarse_voxel).astype(np.uint8)  # (32, 32, 32)
                assert coarse_voxel.shape == (32, 32, 32)
                mask = mask_tensor.detach().cpu().numpy()[0, 0]
                mask = np.round(mask).astype(np.uint8)  # (32x4, 32x4, 32x4)
                self.grids.append(coarse_voxel)
                self.masks.append(mask)
            threestudio.info(f"Finished loading {len(self.grids)} coarse voxels")

            self.grid = torch.from_numpy(self.grids[0]).to(self.device).unsqueeze(0).unsqueeze(0).float()
            self.mask = torch.from_numpy(self.masks[0]).to(self.device).unsqueeze(0).unsqueeze(0).float()
            # -------------------------------------- this block end here ---------------------------------------
    ...
    def get_new_coarse_input_voxel(self):
        idx = random.randint(0, len(self.grids) - 1)
        self.grid = torch.from_numpy(self.grids[idx]).to(self.device).unsqueeze(0).unsqueeze(0).float()
        self.mask = torch.from_numpy(self.masks[idx]).to(self.device).unsqueeze(0).unsqueeze(0).float()
    ...
    def forward(
        ...
        if self.training:
            self.get_new_coarse_input_voxel()
