...
class NeRFVolumeRenderer(VolumeRenderer):
    ...
    def forward(
        ...
        **kwargs
        ...
        # for regularization in 2d space
        if "coarse_density" in geo_out:
            weights_coarse: Float[Tensor, "Nr 1"]
            weights_coarse_, _, _ = nerfacc.render_weight_from_density(
                t_starts[..., 0],
                t_ends[..., 0],
                geo_out["coarse_density"][..., 0],
                ray_indices=ray_indices,
                n_rays=n_rays,
            )
            weights_coarse = weights_coarse_[..., None]
            opacity_coarse: Float[Tensor, "Nr 1"] = nerfacc.accumulate_along_rays(
                weights_coarse[..., 0], values=None, ray_indices=ray_indices, n_rays=n_rays
            )
            out.update(
                {
                    "opacity_coarse": opacity_coarse.view(batch_size, height, width, 1),
                }
            )
